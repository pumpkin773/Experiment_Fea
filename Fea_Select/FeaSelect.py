...
'随机稀疏模型Randomized sparse models'
def RSM(fea, label_dic):
    col = list(fea.columns)
    label_list = list(label_dic.values())
    # 随机稀疏模型Randomized sparse models
    selector = RandomizedLogisticRegression(C=1, scaling=0.5, sample_fraction=0.75, n_resampling=200,
                                                      selection_threshold=0.25, tol=0.001, fit_intercept=True,
                                                      verbose=False, normalize=True, random_state=None, n_jobs=1,
